...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Já existe um usuário cadastrado com o cpf {atleta_cpf}",
        )
    return atleta_out
...
async def query(
    db_session: DatabaseDependency, nome: str | None = None, cpf: str | None = None
) -> Page[AtletaOut]:
    if nome:
        if not cpf:
            ...
            atletas: list[AtletaOut] = (
                (
                    await db_session.execute(
                        select(AtletaModel).filter_by(nome=nome).filter_by(cpf=cpf)
                    )
                )
                .scalars()
                .all()
    ...
    return paginate(db_session, select(Atletas))
...
async def get(db_session: DatabaseDependency, id: UUID4) -> AtletaOut:
    if not id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Nenhum parâmetro informado para busca",
        )
    else:
        atleta: AtletaOut = (
